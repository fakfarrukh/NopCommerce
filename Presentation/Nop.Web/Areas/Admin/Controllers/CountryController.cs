...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CountryController : BaseAdminController
    {
        ...
        private readonly IaaStateProvinceService _aastateProvinceService;
        ...
        public CountryController(IAddressService addressService,
            ...
            IaaStateProvinceService aaStateProvinceService)
        {
            ...
            _aastateProvinceService = aaStateProvinceService;
        }
        ...
        {
            return RedirectToAction("aaList");
        }
        ...
        public virtual async Task<IActionResult> aaList()
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return AccessDeniedView();
        ...
        [HttpPost]
        public virtual async Task<IActionResult> aaStates(StateProvinceSearchModel searchModel)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return await AccessDeniedDataTablesJson();
            ...
            //prepare model
            var model = await _aastateProvinceService.GetStatesByCountryId(searchModel.CountryId);
            dynamic dynamic = new ExpandoObject();
            
           
            List<aaStateProvinceModel> aaStateProvinceModel = new List<aaStateProvinceModel>();
            foreach (var item in model)
            {
                aaStateProvinceModel.Add(new Models.Directory.aaStateProvinceModel()
                {
            ...
            dynamic.Data = aaStateProvinceModel;
        ...
        public virtual async Task<IActionResult> aaStateCreatePopup(int countryId)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return AccessDeniedView();
            ...
            return View("~/Plugins/Appy.Appointment4/Views/StateProvince/aaStateCreatePopup.cshtml", model);
        }
        [HttpPost]
        ...
        [HttpPost]
        public virtual async Task<IActionResult> aaStateCreatePopup(StateProvinceModel model)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return AccessDeniedView();
            ...
            if (ModelState.IsValid)
            {
                var sp = ConvertServiceModel(model);

                await _aastateProvinceService.InsertStateProvinceAsync(sp);
        ...
        public virtual async Task<IActionResult> aaStateEditPopup(int id)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return AccessDeniedView();

            //try to get a state with the specified id
            var state = await _aastateProvinceService.GetStateProvinceByIdAsync(id);
            if (state == null)
                return RedirectToAction("List");
            ...
            if (country == null)
                return RedirectToAction("List");
            StateProvinceModel aaStateProvinceModel = new StateProvinceModel();


            aaStateProvinceModel.CountryId = state.CountryId;
            aaStateProvinceModel.Id = state.Id;
            aaStateProvinceModel.Name = state.Name;
            aaStateProvinceModel.Published = state.IsPublished;
            aaStateProvinceModel.Abbreviation = state.Abbreviation;
            aaStateProvinceModel.ArabicName = state.ArabicName;
            aaStateProvinceModel.DisplayOrder = state.DisplayOrder;
            //prepare model
            //var model = await _countryModelFactory.PrepareStateProvinceModelAsync(null, country, state);
            //return View(aaStateProvinceModel);
            return View("~/Plugins/Appy.Appointment4/Views/StateProvince/aaStateEditPopup.cshtml", aaStateProvinceModel);
        }
        ...
        public virtual async Task<IActionResult> StateEditPopup(StateProvinceModel model)
        {
            ...
            //if we got this far, something failed, redisplay form
            return View("~/Plugins/Appy.Appointment4/Views/StateProvince/aaStateEditPopup.cshtml", model);
        }

        [HttpPost]
        public virtual async Task<IActionResult> aaStateEditPopup(StateProvinceModel model)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageCountries))
                return AccessDeniedView();

            //try to get a state with the specified id
            var state = await _aastateProvinceService.GetStateProvinceByIdAsync(model.Id);
            if (state == null)
                return RedirectToAction("List");
            ...
            if (ModelState.IsValid)
            {
                state = ConvertServiceModel(model);
                await _aastateProvinceService.UpdateStateProvinceAsync(state);
        ...
        public aaStateProvince ConvertServiceModel(StateProvinceModel state)
        {
            aaStateProvince aaStateProvince = new aaStateProvince();
            aaStateProvince.Id = state.Id;
            aaStateProvince.Name = state.Name;
            aaStateProvince.ArabicName = state.ArabicName;
            aaStateProvince.IsPublished = state.Published;
            aaStateProvince.CountryId = state.CountryId;
            aaStateProvince.DisplayOrder = state.DisplayOrder;
            aaStateProvince.Abbreviation = state.Abbreviation;
            return aaStateProvince;
        }
